...
macro_rules! vulkan_error {
  () => {
    $crate::error::VulkanError::Other(format!("vulkan error"))
  };
  ($($arg:tt)*) => {{
    $crate::error::VulkanError::Other(format!($($arg)*))
  }}
}
...
impl Debug {
  pub fn delete(&mut self) {
    if let (Some(debug_utils), Some(debug_messenger)) = (self.debug_utils.take(), self.debug_messenger.take()) {
      unsafe {
